...
import {checkIfContactExists} from "@/lib/strapi/workshopHelper";
    ...
    const formik = useFormik({
        ...
        onSubmit: async (values) => {
            ...
            checkIfContactExists(values.firstname, values.lastname, values.contact.email).then(data => {
                ...
                console.log("check contact data existing", data)
                const dataMapping = {
                    data: {
                        person: {
                ...
                console.log("check contact data", data)
                     ...
                     const config = {
                         method: `${data && data?.msg === "new contact" ? "POST" : "PUT"}`,
                         headers: {
                         ...
                         .then(data => {
                             ...
                             if(data){
                                 ...
                                 fetch('/api/db/participant',{
                                     ...
                                     body: JSON.stringify({id: data.data.documentId, email: data?.data?.contact[0].email}),
                                 })
                             }
